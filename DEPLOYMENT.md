...
## Authentication
The password for the `wikipedialibrarynav` user is stored in the Moderator Tools 1Password vault.
...
```bash
scp wikipedialibrarynav@scp.oclc.org:/wikipedialibrarynav/expert/ezproxy.usr expert/ezproxy.usr
```
...
```bash
scp expert/ezproxy.cfg wikipedialibrarynav@scp.oclc.org:/wikipedialibrarynav/expert/ezproxy.cfg
```
```bash
scp expert/ezproxy.usr wikipedialibrarynav@scp.oclc.org:/wikipedialibrarynav/expert/ezproxy.usr
```

### Set the preprod deployment flag
```bash
scp expert/flag.preprod  wikipedialibrarynav@scp.oclc.org:/wikipedialibrarynav/expert/flag.txt
```
You will then need to wait for the deployment to take place - this can take anywhere between 15 and 30 minutes. You *should* get an email once ready.
...
```bash
scp expert/flag.prod  wikipedialibrarynav@scp.oclc.org:/wikipedialibrarynav/expert/flag.txt
```
You will then need to wait for the deployment to take place - this can take anywhere between 15 and 30 minutes. You *should* get an email once ready.
